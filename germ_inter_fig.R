...
library(cowplot)
ci_fxn <- function(x) {
  y <- as.data.frame(ci(x,method="HDI", ci=0.50)) #gives 50% CrI (using HDI)
  y[1] <- median(x)
  return(y)
}
...
time_inter <- ggplot(pp, aes(x = strat, y = CI, ymin = CI_low, ymax = CI_high, color = origin, group = origin)) + geom_path(position=pd) +
  geom_pointrange( position=pd,size=1, alpha = 1) + scale_x_discrete( limits =c('60','30')) +theme_bw()+
...
pp <- cbind(pp, 'origin' = factor(c('native','introduced','native','introduced'), levels = c('native','introduced')))
pp <- cbind(pp, 'strat' = c("60","60","30","30"))
gr_inter <-  ggplot(pp, aes(x = strat, y = CI, ymin = CI_low, ymax = CI_high, color = origin, group = origin)) + geom_path(position=pd) +
  geom_pointrange( position=pd,size=1, alpha = 1) + scale_x_discrete( limits =c('60','30')) + theme_bw() +
