...
How does our result compare to the `predcomps` package? This package only works or numerical, ordered, or binary inputs. It also only works for linear models. So we'll have to simplify our model a good deal in order to compar this method. Such a simple model cannot satisfactorally model our real data, so we'll first simulate some simpler data: 
```{r}
set.seed(100)
nind=n
norigin = 2 
...
```
These model accurately replicate our input values. 
...
#First making a bayesian model: 
  ...
  (APC<-(num/denom)^(1/2)) #this is the average predictive comparison 
```
The APC value from `predcomps` and from my method are very similar -- any differences likely reflect differences between the Bayesian and frequentist models or the particular model draws used. Thus, my method seems accurate. 
